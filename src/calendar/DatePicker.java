...
public class DatePicker extends HBox {
    ...
    public DatePicker(Locale locale) {
        ...
        textField.setPromptText("YYYY/mm");
    ...
    public void setSelectedDate(LocalDate date) {
        this.selectedDate.set(Date.from(date.atStartOfDay().atZone(ZoneId.systemDefault()).toInstant()));
    }

    public LocalDate getSelectedDate() {
        return (selectedDate.get() == null) ? null :selectedDate.get().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
    }
    ...
    private void showPopup() {
        ...
        double posY = calendarBounds.getMinY() + bounds.getMinY() + getScene().getY() + getScene().getWindow().getY() ;
        popup.show(this, posX, posY);

    }

    public void setIsFromDatePicker(Boolean val){
        calendarView.setIsFromCalendar(val);
    }
}
